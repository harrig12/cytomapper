...
#' @importFrom SummarizedExperiment assay
.cytomapper_server_notabs <- function(object, mask, image, cell_id, img_id,
                               input, output, session, ...)
{
    # Session info observer
    cur_sessionInfo <- sessionInfo()
    ...
    # Dynamically generate scatter plots
    output$AdditionalPlots_tab1 <- .addPlots_tab1_notabs(input)
    
    # Dynamically create image plot
    output$AdditionalPlots_tab2 <- .addPlots_tab2_notabs(input, object, mask, image)
...
#' @importFrom SummarizedExperiment assay
.pickcell_server <- function(object, mask, image, cell_id, img_id,
                             input, output, session, ...)
{
    # Session info observer
    cur_sessionInfo <- sessionInfo()
