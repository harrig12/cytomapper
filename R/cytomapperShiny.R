...
#'Shiny application to visualise gated cells on images
#'
#'Modifeied from \code{\link{cytomapperShiny}} by 
#'Nils Eling (\email{nils.eling@@dqbm.uzh.ch}) and 
#'Tobias Hoch (\email{tobias.hoch@@dqbm.uzh.ch}). 
#'
#'This version displays plots and images side by side
#'
#'@examples
#'## Only run this example in interactive R sessions
#'if (interactive()) {
...
#'
#'    # Use shiny with SCE object, images and masks
#'    cytomapperShiny_notabs(object = pancreasSCE, mask = pancreasMasks, 
#'                  image = pancreasImages, cell_id = "CellNb", 
#'                  img_id = "ImageNb")
#'})
#'
#'@author Cait Harrigan (\email{cait.harrigan@@mail.utoronto.ca})
#'@export
#'@import shiny
#'@import shinydashboard
cytomapperShiny_notabs <- function(object,
                            mask = NULL,
                            image = NULL,
                            cell_id = NULL,
                            img_id = NULL,
    ...
    shiny_ui <- dashboardPage(
        header = .cytomapper_header(),
        sidebar = .cytomapper_sidebar(),
        body = .cytomapper_body_notabs(),
    )
    
    shiny_server <- function(input, output, session) {
        .cytomapper_server_notabs(object, mask, image, cell_id, img_id,
                           input, output, session, ...)
    }
...
#'Shiny application to visualise expression values of cells picked from an image.
#'
#'Modified  from \code{\link{cytomapperShiny}} by 
#'Nils Eling (\email{nils.eling@@dqbm.uzh.ch}) and 
#'Tobias Hoch (\email{tobias.hoch@@dqbm.uzh.ch})
#'
#'This shiny application allows users to pick cells from an image and visualises
#'their raw or transformed expression values. 
#'
#'@param object a \code{\linkS4class{SingleCellExperiment}} object.
#'@param mask (optional) a \code{\linkS4class{CytoImageList}} containing
#'    single-channel \code{\linkS4class{Image}} objects.
...
#'
#'@section Passing further parameters:
#'To customise the visual output of \code{pickCellShiny}, refer to
#'\code{\link{plotting-param}}. Further arguments can be passed to the
#'\code{\link{plotCells}} and \code{\link{plotPixels}} function. As an example:
#'To avoid interpolation of output images, set \code{interpolate = FALSE}.
#'Passing the parameter \code{interpolate = FALSE} will make images less blurry,
...
#'workflow on how to use the app.
#'
#'@seealso \code{\link{cytomapperShiny}}, \code{\link{plotCells}} and 
#'\code{\link{plotPixels}} for the main plotting functions
#'
#'@return A Shiny app object for image picking of cells
#'
#'@examples
#'## Only run this example in interactive R sessions
#'if (interactive()) {
...
pickCellShiny <- function(object,
    ...
    shiny_server <- function(input, output, session) {
        .pickcell_server(object, mask, image, cell_id, img_id,
                         input, output, session, ...)
    }
